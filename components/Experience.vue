<script setup lang="ts">
    ...
    const location = computed(() => props.locations.find(l => l.id === props.experience.location_id))

    const duration = computed(() => {
        const startDate = new Date(props.experience.start_date)
        const endDate = new Date(props.experience.end_date ?? Date.now())

        const diffMonths = (endDate.getMonth() - startDate.getMonth() + (endDate.getFullYear() - startDate.getFullYear())*12 )
...
<template>
    <li class="experience">
        <span class="absolute flex p-1 bg-white rounded-full ring-gray-300 size-11 -start-6">
            <NuxtImg v-if="experience.logo" :src="experience.logo" />
        </span>
        <h3 class="text-lg font-semibold">{{ experience.job_title }}</h3>
        <div class="flex items-center gap-2">
            <UIcon name="i-ph-buildings" />
            <h2 class="text-md">{{ experience.bussiness_name }}</h2>
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-300">
            ...
            <span><span v-if="duration.years > 0">{{  t('year', { value : duration.years}, duration.years) }}</span> {{ t('month', { value: duration.months } ,duration.months ) }}</span>
        </div>
        <p class="pt-3 text-md dark:text-gray-300">{{ experience.description }}</p>
        <div class="flex flex-wrap gap-2 pt-6">
            <UBadge v-for="(skill, i) in experience.skills" :key="`skill_${i}`" :ui="{ rounded: 'rounded-full' }">{{ skill }}</UBadge>
        </div>
    </li>
</template>
